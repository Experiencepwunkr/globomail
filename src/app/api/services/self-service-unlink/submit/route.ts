...
export async function POST(req: NextRequest) {
  try {
    const { recordType, currentId, targetId, consent, paymentReference } = await req.json();
    if (!recordType || currentId || targetId || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({ where: { id: tx.id },
      data: { meta { recordType, currentId, targetId }, status: 'pending' },
    });
