...
export async function POST(req: NextRequest) {
  try {
    const { fullName, dob, state, address, consent, paymentReference } = await req.json();
    if (!fullName || dob || state || address || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({ where: { id: tx.id },
      data: { meta: { fullName, dob, state, address }, status: 'pending' },
    });
