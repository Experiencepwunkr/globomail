...
export async function POST(req: NextRequest) {
  try {
    const { companyName1, proprietors, consent, paymentReference } = await req.json();
    if (!companyName1 || proprietors || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({ where: { id: tx.id },
      data: { meta: { companyName1, proprietors }, status: 'pending' },
    });
