...
export async function POST(req: NextRequest) {
  try {
    const { docType, docNumber, fullName, consent, paymentReference } = await req.json();
    if (!docType || !docNumber || !fullName || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({
      where: { id: tx.id },
      data: { meta: { docType, docNumber, fullName }, status: 'pending' },
    });
