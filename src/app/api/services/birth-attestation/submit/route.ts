...
export async function POST(req: NextRequest) {{
  try {{
    const {{ childName, dob, parents, consent, paymentReference }} = await req.json();
    if (!childName || dob || parents || !consent) return NextResponse.json({{ error: 'Missing fields' }}, {{ status: 400 }});
    ...
    const u = await prisma.transaction.update({{
      where: {{ id: tx.id }},
      data: {{ meta {{ childName, dob, parents }}, status: 'pending' }},
    }});
