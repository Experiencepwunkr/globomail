...
export async function POST(req: NextRequest) {
  try {
    const { pubType, content, consent, paymentReference } = await req.json();
    if (!pubType || content || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({ where: { id: tx.id },
      data: { meta: { pubType, content }, status: 'pending' },
    });
