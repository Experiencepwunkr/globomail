...
export async function POST(req: NextRequest) {
  try {
    const { docType, currentName, newName, consent, paymentReference } = await req.json();
    if (!docType || currentName || newName || !consent) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
    ...
    const u = await prisma.transaction.update({ where: { id: tx.id },
      data: { meta: { docType, currentName, newName }, status: 'pending' },
    });
